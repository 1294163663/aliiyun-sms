...
class SmsServer
{
    ...
    public static function getAcsClient($config = []) {
        ...
        $accessKeySecret = "OPeIWDQx8g4iFvz6DrtZdON3HfZyit"; // AccessKeySecret
     ...
     */
    public static function sendSms($phone, $code,$data, $config = []) {
        ...
        $request->setSignName($config['signature']);
...
//        $request->setSmsUpExtendCode("1234567");
     ...
     * @return mixed|\SimpleXMLElement
     */
    public static function sendBatchSms($phones, $code, $data,$config = []) {
        ...
        $request->setSignNameJson(json_encode($config['signature'], JSON_UNESCAPED_UNICODE));
     ...
     * @return mixed|\SimpleXMLElement
     */
    public static function querySendDetails($phone, $data, $config = []) {
        ...
        $request->setSendDate(isset($data['date']) ?: "20170818");

        // 必填，分页大小
        $request->setPageSize(isset($data['pageSize']) ?: 10);

        // 必填，当前页码
        $request->setCurrentPage(isset($data['page']) ?: 1);

        // 选填，短信发送流水号
        $request->setBizId(isset($data['bizId']) ?: 1);
